...
class Solution:
    def findXSum(self, nums: list[int], k: int, x: int) -> list[int]:
        ...
        for i in range(n-k+1):
            window=nums[i:i+k]
            freq_map=Counter(window)
            top=sorted(freq_map.items(),key= lambda x:(x[1],x[0]),reverse=True)[:x]
            top=set(v for v,_ in top)
            ans.append(sum(num for num in window if num in top))
        return ans 
